...
First import data (see functions)
...
Inspect the data, and note that many columns data have been transformed from their raw form. 
...
Ask Suzanne for raw data so that we can, e.g. model church frequency as a monotonic categorical variable, and pick appropriate sampling distributions for the outcome variables. 
...
Exploration Graphs.  Inspect the data.
...
There are many theories of how religion affects well-being we might consider (cite Highland et al). We will focus on two non-trivial theories -- one related to religious behavior and another related to religious belief. 
...
Before proceeding with the model, let's note a curiosity. If our  interest is in the direct effects of church attendance on well-being we should *not* include social support because this introduces a collider bias! 
...
Let this serve as a warning for us to think about our causation, and what we might want to know before we leap into modeling
...
d1 <- df %>%
  ...
  dplyr::select(anx, ch, mn, bg, anx_s, age_s, ch_s, mn_s, male2, country, male, bg_s)
...
Before racing ahead, lets do some posterior predictive checks
...
When we do a posterior predictive check we find problems in the predicted distribution. This we called this a gaussian, but it is not. The data have large peaks reflecting the categorical qualities of the outcome variable.

We should work with the original, untransformed values of the data. 
...
<!-- ```{r cache = TRUE} -->
<!-- plot(ggeffects::ggpredict(m1_prior, terms = c("ch_s[minmax]"),type = "fe", ci.lvl = 0.89))  +  scale_fill_brewer(palette= "Set2") + labs( title = "Predicted effects of church on anxiety") -->
<!-- ``` -->
...
Nothing happening at the individual level.
...
But lots of variability at the country level intercepts for anxiety. We can see that India is over two standard deviations  less anxious than the average culture level anxiety. By contrast, Japan, Israel, and France are over .2 standard deviations more anxious than average. There looks to be perhaps some covariance between the country intercepts and slopes... maybe though all estimates crosses zero. We'd either want to use stronger priors or collect data from more countries than are in this sample. 
...
Let's delving into this question of country level effects, but asking whether people who are relatively more church attending tend to have more anxiety. 

- within_church centering = relative church attendance within a country.

- between_church centering = the adjustment for country level differences in church attendance.
...
But a between-country effect emerges:
...
plot(ggeffects::ggpredict(
  m1a,
  terms = c("country[all]", "ch_within[minmax]"),
  type = "re",
  ci.lvl = 0.89))  +
  scale_fill_brewer(palette = "Set2") + 
  labs(title = "Predicted effects of church on anxiety: countries X within") + 
...
plot(ggeffects::ggpredict(
  m1a,
  terms = c("ch_between[minmax]"),
  type = "re",
  ci.lvl = 0.89))  +
  scale_fill_brewer(palette = "Set2") + labs(title = "Predicted effects of church on anxiety: between")  + scale_y_continuous(limits = c(-1, 1))
```
...
p<-brms::conditional_effects(m2, "bg_s", 
                              spaghetti = T,
                              nsamples = 300,
                              prob = .89,
                              method = 'fitted')
...
plot(ggeffects::ggpredict(m2, terms = c("country[all]", "bs_s[minmax]"),type = "re", ci.lvl = 0.89))  +
   scale_fill_brewer(palette= "Set2") + labs(
    title = "Predicted effects of bs_s on anxiety") + 
  scale_y_continuous(limits = c(-1,1))
```
...
```{r cache = TRUE}
m2b <- brm(
  anx_s ~ age_s  + male  +  bg_within + bg_between +  (1  +  bg_within|country),
  data = d1,
  family = "gaussian",
  seed = 1234,
  warmup = 1000,
...
p<-brms::conditional_effects(m2b, "bg_within", 
                              spaghetti = T,
                              nsamples = 300,
                              prob = .89,
                              method = 'fitted')
...
plot(p, plot = FALSE)[[1]] + scale_y_continuous(limits = c(-1,1))  + xlab("bg_between") + ylab ("anxiety in sd units") + 
...
```{r cache = TRUE}
plot(ggeffects::ggpredict(m2b, terms = c("country[all]", "bg_within[minmax]"),type = "re", ci.lvl = 0.89))  +
   scale_fill_brewer(palette= "Set2") + labs(
...
To do: 
  ...
  2. Use stronger priors and include stronger prior predictive checks


  3. Sample from an ordinal outcome distribution: obtain un-transformed data from Suzanne
