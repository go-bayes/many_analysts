...
More church attendance predicts greater anxiety

```{r}
parF<-parameters::model_parameters(m1o ) 
...
```{r}
parR<-parameters::model_parameters(m1o,effects = "random" ) 
...
Graphing results wee see that the effects are driven entirely at the high end of the anxiety scale.  This is consistent with a demand characteristic. Anxiety draws people to religion (Sibley and Bulbulia 2012)
...
No clear predictive effect of belief on anxiety
...
```{r}
parF<-parameters::model_parameters( m2o ) 
plot(parF,  sort = TRUE) +theme_lucid() #option -->
```
...
```{r}
parR<-parameters::model_parameters(m2o,effects = "random" ) 
plot(parR,  sort = TRUE) +theme_lucid() #option -->
```

No country level predictions in the slope/intercept covariance:
...
Next, we inspect the data. A challenge is that many columns data have been transformed from their raw form into a number between 0 and 1. Later we'll see that these transformations render the modelling less reliable than if the data were retained.
