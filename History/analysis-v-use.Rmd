...
saveRDS(ipb_bg, here::here("mods","ipb_bg_ln"))
...
p0priorTEST<-brms::conditional_effects(modmc_wt_priorStrong_TEST, "ch_s", 
                              spaghetti = T,
                              ndraws = 1000,
                              categorical = F,
                              prob = .95,
...
church_plot_test_strong <- plot(p0priorTEST, plot = FALSE)[[1]]   +  xlab("Service attendance (standardised)") + ylab ("Anxiety (standardised)") + labs(title = "Prior Sanity Check: Baysian Multi-level Model", subtitle = "Sampling from STRONG Priors Only") + scale_y_continuous(limits = c(-.5,.25))+ 
  theme_classic()
church_plot_test_strong



pred_brms

p0priorTEST_weak<-brms::conditional_effects(modmc_wt_prior_weak, "ch_s", 
                              spaghetti = T,
                              ndraws = 1000,
                              categorical = F,
                              prob = .95,
...
church_plot_test_weak <- plot(p0priorTEST_weak, plot = FALSE)[[1]]   +  xlab("Service attendance (standardised)") + ylab ("Anxiety (standardised)") + labs(title = "Prior Sanity Check: Baysian Multi-level Model", subtitle = "Sampling from WEAK Priors Only") + scale_y_continuous(limits = c(-.5,.5))+ 
  theme_classic()
church_plot_test_weak
...
effect_draws <-  modmc_wt_priorStrong %>% 
  emmeans(~ ch_s,
          at = list(ch_s = c(-0.8321022,2.23193),
                    country = NA),
          epred = TRUE) %>% 
...
# marginal mean 
effect_draws %>% mean_hdi() 

# linear model, for EVALUES
effect_draws_ln <-  modmc_wt_linear %>% 
  emmeans(~ ch_s,
          at = list(ch_s = c(-0.8321022,2.23193),
                    country = NA),
          epred = TRUE) %>% 
...
# marginal mean 
effect_draws_ln %>% mean_hdi() 


average_marginal_effect <- ggplot(autonomy_effect_draws, aes(x = .value)) +
  stat_halfeye(fill = "dodgerblue") +
  labs(x = "Average marginal effect of church attendance", y = "Density") +
  theme_clean() +
...
grand_mean <- modmc_wt_priorStrong %>% 
  epred_draws(newdata = expand_grid(ch_s = seq(minchur, maxchur, by = 0.05)), 
              re_formula = NA)
 ...
 # scale_fill_viridis_d(palette = "Blues") +
  labs(x = "Frequency of religious service attendance (standard deviation units)", 
       y = "Predicted anxiety (standard deviation units)",
...
# sensitivity = Church
evalues.OLS(-0.1093  , se = 0.0172, sd = 1, delta = 1.4)

evalues.OLS(-0.2649 , se = 0.0648, sd = 1, delta = 1)

evalues.OLS(-0.17 , se = 0.05, sd = 1, delta = 1)
#1.61  

# multibias
multi_evalue(est = RR(0.981), biases = selection())
    

#
bias_plot(rr = 0.857, xmax=5)
...
## Bayesian weighted regression correct sample 
...
```{r}
b_ln <- brm(
  anx_s|weights(ipwCH) ~ ch_s + (1 + ch | country),
  data = dr,
    prior = c(
    prior(normal(0, .1), class = Intercept),
    prior(normal(-.1,.1), class = b)),
