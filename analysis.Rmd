...
```{r}
plot(ggeffects::ggpredict(m1, terms = c("country[all]", "ch_s[minmax]"),type = "re", ci.lvl = 0.89))  +  scale_fill_brewer(palette= "Set2") + labs( title = "Predicted effects of church on anxiety")
```


But recall big differences within and between counties in Church Attendence
...
We can use the within and between centred indicators for attendence to clarify how such heterogeneity affects inferenence.

- within_church centering = relative church atttendance within a country

- between_church centering = the adjustment for country level differences in churhc attendance
...
```{r}
plot(ggeffects::ggpredict(m1a, terms = c("ch_within[minmax]"),type = "re", ci.lvl = 0.89))  +
   scale_fill_brewer(palette= "Set2") + labs(
    title = "Predicted effects of church on anxiety")
```

And we do not find reliable heterogeneity at the country level.
```{r}
plot(ggeffects::ggpredict(m1a, terms = c("country[all]", "ch_within[minmax]"),type = "re", ci.lvl = 0.89))  +
   scale_fill_brewer(palette= "Set2") + labs(
    title = "Predicted effects of church on anxiety")
```
...
parameters::model_parameters(m1_a)
parameters::random_parameters(m1_a)

```

Within countries we do not find  effects from different levels of church attendance on anxiety
...
And we do not find reliable heterogeneity at the country level.
```{r}
plot(ggeffects::ggpredict(m1_a, terms = c("country[all]", "ch_within[minmax]"),type = "re", ci.lvl = 0.89))  +
   scale_fill_brewer(palette= "Set2") + labs(
    title = "Predicted effects of church on anxiety")
```

Countries who differ in their average levels of church attendence differ present differences in their expected anxiety levels. Those with more attendence are expected to present less anxiety.
```{r}
...
And we do find thi separation at the country level. 
```{r}
...
Countries who differ in their average levels of church attendence differ present differences in their expected anxiety levels. Those countries greater levels of church attendance may be expected to present less anxiety.


Take homes:

- No evidence for a "relative" church attendence effect. 
- Evidence for a between country effect, however this  is *only* intereting if you are a sociologist
- Sociological data, averaged at the country level, is unhelpful for psychological inference at the individual level. This point is relevant to big historical database projects that are collecting country and culture level data. Although it may be interesting to have sociological interests -- e.g. in whether countries where people go to church more often have lower levels of anxiety, we must not confuse this question with an individual level question about the psychological effects of church attendance. 
- A key point to keep in mind: the reasons people go to churh might differ. For example church attendance might reduce anxiety and for this reason people attend.  Without understanding the demand characteristics for church attendance we are likely to get the wrong inference. Consider an analogy. Imagine we modelled the relationship between taking antiobotics and infection. Without knowning why people were taking antiobiotics we might be tempted into inferring inferring from cross sectional data that antiobotics *cause* infection.  For this reason, it is very important to collect longitudional data on people, and to focus on whether there are demand qualities that lead people to attend church. 

Next we turn to the belief in God model. 
...
```{r}
m2_a <- brm(
  anx_s ~ age_s  + male +  bg_between + (1 + bg_within|country + bg_between | country),  
  data = d1,
  family = "gaussian",
  seed = 1234,
  warmup = 1000,
  ...
  file = "m2_a.rds")
summary(m2_a)
model_parameters(m2_a)
random_parameters(m2_a)
```
