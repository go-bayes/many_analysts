...
d1 <- df %>%
  ...
  dplyr::select(anx, anx_s, age_s, ch_s, mn_s, fm1m2, country, male, bg_s) %>%
  dplyr::filter(
    !is.na(anx_s),
    !is.na(age_s),
    !is.na(ch_s),
...
To do -- look at between country effects and relative effects within countries
...
m_ch <- ulam(
  alist(
    anx_s ~ normal(mu , sigma),
    mu <-  alpha + b1 * ch_s + b2 * age_s  + b3 * male + a_country[country] + b_country[country] * ch_s,
    alpha ~ normal(0,1),
    b1 ~ dlnorm(0,1),
    b2 ~ normal(0,1),
    b3 ~ normal(0,1),
    u_country ~ dnormal(0,1),
    c(a, b)[country] ~ multi_normal(c(a, b) , Rho , sigma_country),
    a ~ normal(0, 2),
    b ~ normal(-1, 0.5),
