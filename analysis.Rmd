...
d1a<-ggdag::ggdag_adjustment_set(dg1a, node_size = 14, text = FALSE, use_labels = "label") +
  ...
  theme_dag() + ggtitle("If we assume distressing events increase Religious Belief, 
Religious Service as well as anxiety, crossectional data cannot avoid confounding ")
...
ggdag::ggdag(dgz) + theme_dag()


# graph adjustment sets
dg1<-ggdag::ggdag_adjustment_set(dg1z, node_size = 14, text = FALSE, use_labels = "label") +
  theme(legend.position = "bottom") +
...
```{r}
library(patchwork)
 (dp0 + d1a)/   (dg1 + dp2)  +   plot_layout(guides = 'collect')  + plot_annotation(title = 'If distressing life events affect religious belief and behaviour, longtidinal data are needed for causal inference', tag_levels = "a") 
# dag_ful 15/15
```
