...
dp0 <-ggdag::ggdag(dg1, text = FALSE, use_labels = "label")  + theme_dag() + ggtitle("Assumed Causal Relations")
# graph adjustment sets
...
# graph adjustment sets
dp2<-ggdag::ggdag_adjustment_set(dg2, node_size = 14, text = FALSE, use_labels = "label") +
  theme(legend.position = "bottom") +
...
```{r}
library(patchwork)
( (dp0)/(dp1 + dp2) +   plot_layout(guides = 'collect'))  +((d0a) + (d1a)) + plot_annotation(title = 'If distressing life events affect religious belief and behaviour, longtidinal data are needed for causal inference', tag_levels = "a")
# 14/12
```
...
belief_random <- plot(parR1,  sort = TRUE) +theme_lucid()  + labs(title = "Country level heterogeneity in belief (co)-variances: India is an outlier.") 
```
...
belief_fixed <- plot(p0, plot = FALSE)[[1]]   + xlab("belief (standardised)") + ylab ("anxiety (ordinal 1-5)") +
  labs(title = "Increasing belief predicts increasing anxiety")
```
...
church_random <- plot(parcR,  sort = TRUE) +theme_lucid() + labs(title = "Country level heterogeneity in religious service (co)-variances: India is again an outlier.") #option -->
```
...
church_fixed <-plot(p0b, plot = FALSE)[[1]]  + xlab("religious service attendance (standardised)") + ylab ("anxiety (ordinal 1-5)") +
  labs(title = "Increasing religious service attendence predicts lower anxiety.")
...
```{r}
(belief_fixed + church_fixed )/( belief_random + church_random  ) +
...
parameters::parameters(
  ...
  diagnostic = "Rhat") %>%
    ...
    kable(booktabs = T, "latex", caption =  "Belief in God is positively related to anxiety") %>%
    print()
```
...
parameters::parameters(
  ...
  diagnostic = "Rhat") %>%
    mutate_if(is.numeric, ~ round(., 2)) %>%
    mutate_all(funs(str_replace(., "b_", ""))) %>%
    kable(booktabs = T, "latex", caption =  "Religious Service is negatively related to anxiety") %>%
    print()
```
...
B <- rnorm( n , C + U ) )# belief increase with service + distressing events
...
Y <- rnorm( n , U - C ) # measured distress caused by distress, reduced by church attendance. 
# simulate data
simdat <- data.frame(Belief = B, # belief
  ReligiousService = C, # religious service
  U = U, # distressing events (unmeasured)
  Anxiety = Y)
